...
table = PrettyTable(["n", "xn", "f(xn)", "f(xn+1)", "relative_error"])


def secant_method(func, x1, x2, tol=0.01, max_iter=100):
    x_prev = x1
    x_curr = x2
    iteration = 0
    relative_errors = []
    ...
    while abs(func(x_curr)) > tol and iteration < max_iter:
        ...
        table.add_row([iteration, x2, x_curr, x_next, relative_error])
